...
#name	week_day	month	day	time	year	lat	lng	Connected_to_HBS	hbsRSS	hsuRSS	hbsEstTput	hsuEstTput	winlink1000OduAirHssCurrentOpState	winlink1000OduAirTotalTxPower	winlink1000HsuAirState	winlink1000HsuAirLinkState	winlink1000OduAirRxPower	my_hmu1	state	summary	timestamp	PC_timestamp 
 ...
 valid_26 = row.include?('name') && row.include?('Connected_to_HBS') && row.include?('lat') && row.include?('lng') && row.include?('timestamp') && row.include?('hbsRSS') && row.include?('hsuRSS') && row.include?('winlink1000HsuAirState') && row.include?('Connected_to_HBS ') && row.include?('state')
 return valid24||valid_26
 end
...
#    out += "	<description><![CDATA[RSSI=#{@rss_BH_AIR_MinRSL||'N/A'}<br>\nPing to Remote=#{@ping_to_remote||''}<br>\nState=#{@state==nil ? '' : @state.strip}<br>\nConnected_to_HBS=#{@_connected_to_HBS||''}"+
    ...
    puts @error_message
    puts self.inspect
  end #end of begin_rescue   
  end #def_to_kml
  
 end#def_intialize
  
end #end of location_24
...
def main_24(row)
 ...
 case col_name.to_s.downcase.strip
   ...
   @locations << Location_24.new(:name => "#{rows}@", :address => row[address], :lat => row[lat], :lng => row[lng],  :timestamp => row[timestamp], :BH_AIR_MinRSL=> row[rss_BH_AIR_MinRSL], :BH_AIR_MaxRSL=> row[rss_BH_AIR_MaxRSL], :state=> row[state], :connected_to_HBS => row[connected_to_hbs], :ping_to_remote  => row[ping_to_remote])
   #  puts @locations.last.to_kml
   if DEBUG then
     puts ("parsing row:#{row}");
	 puts @locations.last.to_kml
   end    
 end #end of main_24
...
f.write("</Document>\n</kml>")
 ...
 begin
   ...
   puts @options
   puts ("parsing file: #{@options[:inputfile]} creating file: #{@options[:myfilename]}")
   CSV.foreach(@options[:inputfile]) do |row|
     ...
     if first 
       ...
       if !file_is_valid(row)
        ...
        main_24(row) if valid_24;
	    main_26(row) if valid_26;
	  end  #if first line
   end # do row	  
 rescue
    @error_message="#{$!} ";     
