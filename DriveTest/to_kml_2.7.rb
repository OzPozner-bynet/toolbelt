...
DOCTYPE = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n
 ...
 opts = OptionParser.new do |opts|
    opts.banner = 'To KML version 2.7'
  ...
  inputfilename=@options[:inputfile].split(".",2)
  @options[:myfilename] = inputfilename[0]+'.kml' if inputfilename[1]=="csv";
...
hsuRSS , hbsRSS , winlink1000HsuAirState , valid_24, valid_26 = nil, nil , nil , nil , nil
...
def file_is_valid(row)
 ...
 puts ("valdite header")
 valid_24 = row.include?('name') && row.include?('address') && row.include?('lat') && row.include?('lng') && row.include?('timestamp') && row.include?('BH_AIR_MinRSL') && row.include?('BH_AIR_MaxRSL') && row.include?('state') && row.include?('Connected_to_HBS ') && row.include?('ping to remote')
  puts "valid_24" if valid_24
#name	week_day	month	day	time	year	lat	lng	Connected_to_HBS	hbsRSS	hsuRSS	hbsEstTput	hsuEstTput	winlink1000OduAirHssCurrentOpState	winlink1000OduAirTotalTxPower	winlink1000HsuAirState	winlink1000HsuAirLinkState	winlink1000OduAirRxPower	my_hmu1	state	summary	timestamp	PC_timestamp 

 valid_26 = row.include?('name') && row.include?('lat') && row.include?('lng') && row.include?('timestamp') && row.include?('hbsRSS') && row.include?('hsuRSS') && row.include?('winlink1000HsuAirState') && row.include?('Connected_to_HBS ') && row.include?('state')
 if DEBUG then
   puts "not valid header #{valid_26}" 
   puts "name #{row.include?('name')}" 
   puts "lat #{row.include?('lat')}"
   puts "lng #{row.include?('lng')}"
   puts "timestamp #{row.include?('timestamp')}" 
   puts "hbsRSS #{row.include?('hbsRSS')}"
   puts "hsuRSS #{row.include?('hsuRSS')}"
   puts "Connected_to_HBS #{row.include?('Connected_to_HBS')}"
   puts "state #{row.include?('state')}"
   puts "winlink1000HsuAirState #{row.include?('winlink1000HsuAirState')}"
 end
   return true
...
class Location
 ...
 #Valid_26
  attr_accessor :name, :address, :lat, :lng, :timestamp ,:state , :connected_to_HBS , :hbsRSS, :hsuRSS , :winlink1000HsuAirState
  ...
  def initialize(myoptions = {})
   ...
   #   @address = myoptions[:address]
      @hbsRSS = myoptions[:hbsRSS]
	  @hsuRSS = myoptions[:hsuRSS]
	  @winlink1000HsuAirState = myoptions[:winlink1000HsuAirState]
	  @lat = myoptions[:lat]
      @lng = myoptions[:lng]
 ...
 #2.4#     if (@address == "0") && not( @ping_to_remote == "0") then
 #2.4#	     @address = @rss_BH_AIR_MaxRSL
 #2.4#	  end
...
begin 
	...
	#2.4#     out +=  "[#{@address}]" unless !displayrss
		      out +=  "[SU:#{@hsuRSS}][BS:#{@hbsRSS}]" unless !displayrss
...
#	  rss_styles = Array.new [ -30,-50,-60 ,-65,-68,-70,-73,-75,-78,-79,-80,-81,-83 ,-84,-85,-90,-95,-105,-121 ]  
    ...
    #    if @rss_BH_AIR_MinRSL.to_i <= @rss_styles[index].to_i then 
	 #2.4#   if @address.to_i <= @@rss_styles[index].to_i then 
     #2.4#     style_index = (index==0)? '': index.to_s;
     #2.4#   end
		if @hsuRSS.to_i <= @@rss_styles[index].to_i then 
...
#2.4    out += "	<description><![CDATA[MAX=#{@rss_BH_AIR_MaxRSL||'N/A'}<br>MIN=#{@rss_BH_AIR_MinRSL||'N/A'}<br>RSSI=#{@address||'N/A'}<br>\nPing to Remote=#{@ping_to_remote||''}<br>\nState=#{@state==nil ? '' : @state.strip}<br>\nConnected_to_HBS=#{@_connected_to_HBS||''}"+
     out += "	<description><![CDATA[hsuRSS=#{@hsuRSS||'N/A'}<br>hbsRSS=#{@hbsRSS||'N/A'}<br>\nwinlink1000HsuAirState=#{@winlink1000HsuAirState||''}<br>\nState=#{@state==nil ? '' : @state}<br>\nConnected_to_HBS=#{@_connected_to_HBS||''}"+
    ...
    "<ExtendedData>\n" +
      ...
      "</Data>\n" +
        ...
        "<value>#{@hsuRSS}</value>\n" +
      "</Data>" +
      '<Data name="winlink1000HsuAirState">\n' + 
        "<value>#{@winlink1000HsuAirState}</value>\n" +
      "</Data>" +
    "</ExtendedData>" +
        "   <Point>\n"+
 ...
 CSV.foreach(@options[:inputfile]) do |row|
  ...
  if first 
	...
	#valid_26 = row.include?('name') && row.include?('Connected_to_HBS')
	#&& row.include?('lat') && row.include?('lng') && row.include?('timestamp') && row.include?('hbsRSS') 
	#&& row.include?('hsuRSS') && row.include?('winlink1000HsuAirState') && row.include?('Connected_to_HBS ') 
	#&& row.include?('state')
      row.each_with_index do |col_name, i|
        case col_name.to_s.downcase.strip
          when 'name' 
            name = i
      	    puts ("name in col:#{i}");
		  when 'hsurss' #valid_26
      	    hsuRSS = i
             puts ("hbsRSS in col:#{i}");	
      	  when 'hbsrss' #valid_26
      	    hbsRSS = i
             puts ("hbsRSS in col:#{i}");
		  when 'winlink1000hsuairstate' #valid_26
      	    winlink1000HsuAirState = i
             puts ("winlink1000HsuAirState in col:#{i}");	 
          when 'timestamp'
      		  timestamp = i
     ...
     #2.4# @locations << Location.new(:name => "#{rows}@", :address => row[address], :lat => row[lat], :lng => row[lng],  :timestamp => row[timestamp], :BH_AIR_MinRSL=> row[rss_BH_AIR_MinRSL], :BH_AIR_MaxRSL=> row[rss_BH_AIR_MaxRSL], :state=> row[state], :connected_to_HBS => row[connected_to_hbs], :ping_to_remote  => row[ping_to_remote])
       @locations << Location.new(:name => "#{rows}@", :hsuRSS => row[hsuRSS], :lat => row[lat], :lng => row[lng],  :timestamp => row[timestamp], :hbsRSS => row[hsuRSS], :winlink1000HsuAirState=> row[winlink1000HsuAirState], :state=> row[state], :connected_to_HBS => row[connected_to_hbs])
...
#	if location.hbsRSS.to_i > -20 then index+=1; next; end;
#	if location.hsuRSS.to_i > -20 then index+=1; next; end;
...
#	if (not(DISPLAY_ZERO_RSS) && (location.hbsRSS == "0")) then index+=1; next end;
#	if (not(DISPLAY_ZERO_RSS) && (location.hsuRSS == "0")) then index+=1; next end;
